import DateFormat from '@/components/common/formater/date-format';
import { Column } from '@/components/common/table/TableManageMent';
import { IOverWeight } from '@/types';
import OverWeightActions from './OverWeightActions';

const OverWeightTableColumn: Column<IOverWeight>[] = [
  {
    header: 'SI',
    accessor: (_, i) => i + 1,
  },
  {
    header: 'Patient ID',
    accessor: (p) => p.patientId,
  },
  {
    header: 'Name',
    accessor: (p) => p.name,
  },
  {
    header: 'Phone',
    accessor: (p) => p.phone,
  },
  {
    header: 'Emergency Contact',
    accessor: (p) => p.emergencyContact,
  },

  {
    header: 'User Join Date & Time',
    accessor: (p) => <DateFormat date={p.createdAt} />,
  },
  {
    header: 'Actions',
    accessor: (p) => <OverWeightActions patient={p} />,
  },
];

export default OverWeightTableColumn;
