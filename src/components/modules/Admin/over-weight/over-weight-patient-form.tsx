...
import { createOverWeightPatient } from '@/services/patient-management/over-weight';
...
const OverWeightMultiStepForm = () => {
  ...
  const onSubmit = async (data: OverWeightFormValues) => {
    await executePost({
      action: () => createOverWeightPatient(data),
      success: {
        onSuccess: () => {
          form.reset();
          setStep(1);
        ...
        loadingText: 'Creating Overweight Patient...',
        redirectPath: '/admin/over-weight',
      },
    });
  };
...
export default OverWeightMultiStepForm;
